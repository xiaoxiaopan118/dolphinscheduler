...
public class WorkflowExecutionRunnable implements IWorkflowExecutionRunnable {
    ...
    public void start() {
        ...
        eventRepository.storeEventToTail(new WorkflowTriggeredEvent(getWorkflowInstanceId()));
    }

    @Override
    public void pause() {
        dagEngine.getWorkflowExecutionDAG().getActiveTaskExecutionRunnables().forEach(taskExecuteRunnable -> dagEngine
